...
function modulo() {
  ...
  function es_archivo_debian(nombre_de_archivo) {
    var patron = /README.Debian$|changelog$|control$|.desktop$|.install$/g
    var valor = patron.test(nombre_de_archivo);

    return valor;
  }

  function es_lanzador(nombre_de_archivo) {
    var patron = /bin\/TXT_NOMBRE$/g
    var patron2 = /bin\/TXTNOMBRE$/g
    var valor = (patron.test(nombre_de_archivo) || patron2.test(nombre_de_archivo));
  ...
  function _copyFileSync(source, target, nombre) {
    ...
    target = target.replace("TXTNOMBRE", nombre);
    ...
    if (es_archivo_de_texto(source) || es_archivo_debian(source) || es_lanzador(source)) {
      fs.createReadStream(source).
        pipe(replaceStream('TXT_NOMBRE', nombre)).
        pipe(replaceStream('TXTNOMBRE', nombre)).
